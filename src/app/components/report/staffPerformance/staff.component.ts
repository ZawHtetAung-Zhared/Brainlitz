  ...
  getFilteredDataGroupByLocation(data) {
    ...
    function getLocationData(data) {
      ...
      data.forEach(function(location) {
        ...
        obj.averageRating =
          parseFloat((obj.ratingWeightage / obj.totalRating).toFixed(2)) || 0;
  ...
  getFilteredDataGroupByCategory(data) {
    ...
    function getCategoryData(data) {
      ...
      data.forEach(function(category) {
        ...
        obj.totalRating = _self.getTotalRating(obj.rating);
        obj.ratingWeightage = _self.getRatingWeightage(obj.rating);
        obj.averageRating = parseFloat(
          (obj.ratingWeightage / obj.totalRating).toFixed(2)
        );
        if (obj.totalRating > 0) {
          result.push(obj);
        }
  ...
  mergeDuplicateObject(data) {
    ...
    data.forEach(function(value) {
      var existing = result.filter(function(v, i) {
        return (
          v.groupTypeValue == value.groupTypeValue &&
          v.filterValue == value.filterValue
        );
      });
      ...
      if (existing.length) {
        ...
        result[existingIndex].totalRating = _self.getTotalRating(
          result[existingIndex].rating
        );
        result[existingIndex].ratingWeightage = _self.getRatingWeightage(
          result[existingIndex].rating
        );
        result[existingIndex].averageRating = parseFloat(
          (
            result[existingIndex].ratingWeightage /
            result[existingIndex].totalRating
          ).toFixed(2)
        );
      } else {
        result.push(value);
