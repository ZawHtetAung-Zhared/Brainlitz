...
declare var $: any;
@Component({
  selector: 'app-test-werkz-tools',
  templateUrl: './test-werkz-tools.component.html',
  styleUrls: ['./test-werkz-tools.component.css']
})
export class TestWerkzToolsComponent implements OnInit {
  ...
  somethingChanged(val, name) {
    ...
    // console.log(this.performanceDemands);
    if (
      (this.pickedTag.state != '' && this.pickedTag.state == 'conceptCreate') ||
      this.pickedTag.state == ''
    ) {
      this.conceptCreate = true;
      this.addPd();
    } else if (
        ...
        for (var i = 0; i < pd.length; i++) {
          for (var j = 0; j < pd[i].questions.length; j++) {
            if (pd[i].questions[j].html) {
              document.getElementById('q-' + i + '-' + j).innerHTML =
                pd[i].questions[j].html.question;
            }
          }
        }
      }, 200);
  ...
  getAllContent(page, size, keyword) {
    ...
    return new Promise((resolve, reject) => {
      this._service
        .getContent(this.regionID, page, size, keyword, this.contentType)
        .subscribe(
          (res: any) => {
            this.result = res;
            this.contentRes = res;
  ...
  onslectedImgDiv(i, img) {
    ...
    if (this.modelType == 'single') {
      ...
      if (this.isRemove) {
        ...
        this.selectedImgArr.splice(
          this.selectedImgArr.map(x => x._id).indexOf(i),
          1
        );
  ...
  onslectedVideoDiv(i, video) {
    ...
    if (this.isRemove) {
      ...
      if (this.vidIdArr.includes(i)) {
        this.selectedVideoArr.splice(
          this.selectedVideoArr.map(x => x._id).indexOf(i),
          1
        );
        this.vidIdArr.splice(this.vidIdArr.indexOf(i), 1);
      } else {
        ...
        this.getAllContent(1, 20 * this.contentPage, '').then(() => {
          setTimeout(() => {
            if (this.modelType == 'multiple') {
              ...
              this.selectedImgArr.splice(
                this.selectedImgArr.map(x => x._id).indexOf(id),
                1
              );
    ...
    if (this.isVideo(element)) {
      if (type == 'toString') {
        ...
        if (typeof element.start == 'number') {
          timeString = String(
            new Date(element.start * 1000).toISOString().substr(11, 8)
          );
        ...
        if (typeof element.end == 'number') {
          timeString = String(
            new Date(element.end * 1000).toISOString().substr(11, 8)
          );
          ...
          timeString = String(
            new Date(element.duration * 1000).toISOString().substr(11, 8)
          );
          console.log('time string', timeString);
          res = timeString.split(':');
          element.end = `${res[0]}h ${res[1]}m ${res[2]}s`;
        ...
        let total =
          Number(res[0].slice(0, -1)) * 3600 +
          Number(res[1].slice(0, -1)) * 60 +
          Number(res[2].slice(0, -1));
  ...
  insertImg() {
    ...
    if (this.editableId != '') {
      ...
      if (this.modelType == 'video') {
        ...
        var contArr = this.performanceDemands[this.focusType.parentIdx]
          .questions[this.focusType.no].contents;
      ...
      if (this.modelType == 'single') {
        ...
        var contArr = this.performanceDemands[this.focusType.parentIdx]
          .questions[this.focusType.parentQuesIdx].answers[this.focusType.no]
          .contents;
        Array.prototype.push.apply(contArr, this.selectedVideoArr);
        this.performanceDemands[this.focusType.parentIdx].questions[
          this.focusType.parentQuesIdx
        ].answers[this.focusType.no].contents.forEach(element => {
          console.log('~~~', element);
          this.changeTimeFormat(element, 'toString');
        });
        console.log(this.performanceDemands);
        this.isCollapseVid(
          this.performanceDemands[this.focusType.parentIdx].questions[
            this.focusType.parentQuesIdx
          ].answers[this.focusType.no].contents
        );
  ...
  onFocus(type, idx1, idx2, idx3) {
    ...
    switch (type) {
      ...
      case 'question':
        ...
        // this.isCollapse = true;
        var quesContent = this.performanceDemands[idx1].questions[idx2]
          .contents;
        ...
        break;
      case 'check':
        var ansContent = this.performanceDemands[idx1].questions[idx2].answers[
          idx3
        ].contents;
  ...
  delete(itemType) {
    ...
    if (itemType.type == 'pd') {
      ...
      if (
        this.performanceDemands[itemType.parentIdx].questions[
          itemType.parentQuesIdx
        ].answers.length > 1
      ) {
        this.performanceDemands[itemType.parentIdx].questions[
          itemType.parentQuesIdx
        ].answers.splice(itemType.no, 1);
      }
    }
    this.focusType = {};
    this.showSettingSidebar = false;
  ...
  turn(qId, fType) {
    ...
    setTimeout(() => {
      ...
      if (tempStr.includes('![](./assets/images/remove-white.png)')) {
        markdownQues = this.replaceAll(
          '![](./assets/images/remove-white.png)',
          '',
          tempStr
        );
  ...
  getSingleConcept(cID) {
    ...
    _that._service.getConceptById(_that.regionID, cID).subscribe(
      (res: any) => {
  ...
  getPDObject(_that, pdObj, callback) {
    ...
    _that._service.getPDById(_that.regionID, pdObj).subscribe(
      res => {
        // console.log(res)
        callback(null, res);
      },
  ...
  getQuesById(_that, pdIndex, result, Id, callback) {
    ...
    _that._service.getQuesById(_that.regionID, Id).subscribe(
      res => {
        // console.log(res);
        callback(null, res);
      },
  ...
  conceptDelete(conceptId) {
    ...
    this.modalReference.close();
    this._service
      .deleteConcept(this.regionID, conceptId)
      .subscribe((res: any) => {
  ...
  isVideo(img) {
    ...
    if (testString.includes('video')) return true;
    else return false;
  }
  ...
  deleteSettingContents(i) {
    ...
    if (this.focusType.type == 'pd') {
      ...
      this.performanceDemands[this.focusType.no].contents.forEach(
        (element, ind) => {
          if (this.isVideo(element)) {
            console.log(ind);
            videoArr.push(ind);
      ...
      this.performanceDemands[this.focusType.parentIdx].questions[
        this.focusType.parentQuesIdx
      ].answers[this.focusType.no].contents.splice(i, 1);
      this.isCollapseVid(
        this.performanceDemands[this.focusType.parentIdx].questions[
          this.focusType.parentQuesIdx
        ].answers[this.focusType.no].contents
