...
export class RescheduleComponent implements OnInit {
  ...
  checkAvaiableReschedule(loadmore){
    ...
    for(let i=0;i<this.reScheduleLists.length;i++){
      let endDate=this.datePipe.transform(this.reScheduleLists[i].endDate,'yyyy-MM-dd');
    ...
    this.checkObjArr.emit(this.lessionIdArr);
  ...
  lessonCheck(id, obj) {
    ...
    if(this.lessionIdArr.length < this.avaiableLessonsCount){
      ...
      console.log('else');
      for(let i=0;i<this.reScheduleLists.length;i++){
        const index = this.lessionIdArr.findIndex(
          id => id === this.reScheduleLists[i]._id
        );
        console.log((index));
        if(index != -1){
          this.reScheduleLists[i].isCheck=true;
  ...
  loadmoreReschedule(courseId, uId, lessons) {
    let date = new Date(this.reScheduleLists[this.reScheduleLists.length - 1].startDate);
    ...
    this._service
      .getRescheduleList(this.course._id, this.selectedCustomer.userId, startDate, undefined)
      .subscribe((res: any) => {
