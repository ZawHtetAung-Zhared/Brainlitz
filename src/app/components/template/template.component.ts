...
export class TemplateComponent implements OnInit {
  ...
  getAllAp(id) {
    ...
    this.newAPs = [];
    this._service.getAllAPmodule(this.regionID, id).subscribe(
      (res: any) => {
        //this.blockUI.stop();
        console.log('all ap => ', res);
        this.apLists = res;
        for (let i = 0; i < this.apLists.length; i++) {
          this.apLists[i]['checked'] =
            this.apLists[i].checked == undefined ? false : true;
        }
        if (this.lol == id) {
  ...
  checkedaps(option, e) {
    ...
    this.tempApCount = this.tempApCount.filter(
      f => !this.demoApCount.includes(f)
    );
  ...
  createAP(obj, moduleID) {
    ...
    console.log(obj);
    this._service.createAP(this.regionID, this.locationID, obj).subscribe(
      (res: any) => {
        console.log(res);
  ...
  createTemplate(data, update, id) {
    ...
    if (update == true) {
      ...
      this._service.createTemplate(this.regionID, obj).subscribe(
        (res: any) => {
          console.log(res);
          this.toastr.success('Successfully Created.');
          //this.blockUI.stop();
  ...
  getAllTemplate() {
    //this.blockUI.start('Loading...');
    var moduleId;
    this._service.getAllTemplate(this.regionID, 20, 0, moduleId).subscribe(
      (res: any) => {
  ...
  deleteTemplate(tempid) {
    //this.blockUI.start('Loading...');
    this.modalReference.close();
    this._service.deleteTemplate(this.regionID, this.currentId).subscribe(
      (res: any) => {
        //this.blockUI.stop();
  ...
  shareModal(sharemodal, id) {
    console.log(id);
    this.modalReference = this.modalService.open(sharemodal, {
      backdrop: 'static',
      windowClass: 'animation-wrap'
    });
  ...
  callUpdate(obj, status) {
    //this.blockUI.start('Loading...');
    this._service.updateSingleTemplate(this.regionID, obj).subscribe(
      (res: any) => {
        console.log(res);
        this.getAllTemplate();
